...
void init_gstreamer(AppData* app, const std::string& rtsp_url, const std::string& local_video_path, int width, int height) {
    ...
    std::string local_pipeline = "filesrc location=" + local_video_path + " ! decodebin ! videoconvert ! videoscale ! video/x-raw,format=RGB,width=" + std::to_string(width) + ",height=" + std::to_string(height) + " ! appsink name=appsink_local";
    app->pipeline_local = gst_parse_launch(local_pipeline.c_str(), NULL);
